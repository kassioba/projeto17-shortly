...
export async function getUrl(req, res){
    ...
    try{
        const findUrl = await db.query(`SELECT id, shortUrl, url FROM urls WHERE id=$1`, [urlId])
...
export async function redirectToShortUrl(req, res){
    ...
    try{
        const findShortUrl = await db.query(`SELECT url, visitCount FROM urls WHERE shortUrl=$1`, [shortUrl])

        if(!findShortUrl.rows[0]) return res.sendStatus(404)

        await db.query(`UPDATE urls SET visitCount='${findShortUrl.rows[0].visitcount + 1}' WHERE shortUrl=$1`, [shortUrl])
...
export async function deleteUrl(req, res){
    ...
    try{
        const findToken = await db.query(`SELECT * FROM sessions WHERE token='${token}'`)

        if(!findToken.rows[0]) return res.sendStatus(401)

        const urlUserId = await db.query(`SELECT userId FROM urls WHERE id=$1`, [urlId])

        if(!urlUserId.rows[0]) return res.sendStatus(404)

        if(findToken.rows[0].userid !== urlUserId.rows[0].userid) return res.sendStatus(401)
